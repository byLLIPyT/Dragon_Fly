...
class GameScene: ParentScene {
    ...
    //fileprivate var currentNumberOfEnemies: Int
...
//            self.addChild(powerUp)
    ...
    fileprivate func spawnEnemiesLines() {
    ...
    fileprivate func spawnEnemiesRound() {
...
extension GameScene: SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if contact.bodyA.node?.parent != nil && contact.bodyB.node?.parent != nil {
            ...
            if contact.bodyA.node?.name == "bluePowerUp" {//поймать синий усилитель
                contact.bodyA.node?.removeFromParent()
                lives = 3
                player.bluePowerUp()
            } else if contact.bodyB.node?.name == "bluePowerUp" {//поймать синий усилитель
                contact.bodyB.node?.removeFromParent()
                lives = 3
                player.bluePowerUp()
            }
